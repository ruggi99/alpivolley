...
export default function useClassificaAvulsa(data) {
...
function calcSingolaClassifica(data, i) {
  ...
  for (const row of data) {
    ...
    classifica[index1][EnumClassificaRev.Vittorie_3_0] += points[0] > 2 ? 1 : 0;
    classifica[index2][EnumClassificaRev.Punti] += points[1];
    classifica[index2][EnumClassificaRev.Vittorie] += points[1] > 1 ? 1 : 0;
    classifica[index2][EnumClassificaRev.Vittorie_3_0] += points[1] > 2 ? 1 : 0;
  }
...
function sortClassifica(a, b) {
  ...
  // ordino per vittorie
  if (a[EnumClassificaRev.Vittorie_3_0] != b[EnumClassificaRev.Vittorie_3_0])
    return (
      b[EnumClassificaRev.Vittorie_3_0] - a[EnumClassificaRev.Vittorie_3_0]
    );
