...
function calculateNodesOld(data, max_fasi) {
  const nodes = calculateNodes1(data, max_fasi);
  // Fase diretta
...
export function calculateNodes(data, max_fasi) {
  const nodes = [];
  // Fase diretta
  const NUMERO_FASI = Math.min(Object.keys(Object.groupBy(data, (v) => [v.Fase])).length, max_fasi);
