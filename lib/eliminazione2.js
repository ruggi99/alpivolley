import { getBezierPath, Position } from "@xyflow/react";

import { algoritmo } from "lib/algoritmo";

const FASI = [
  "Finali",
  "Semifinali",
  "Quarti",
  "Ottavi",
  "Sedicesimi",
  "Trentaduesimi",
];

export function calculateData(NUMERO_FASI) {
  const data = [];
  for (let fase = 0; fase < NUMERO_FASI; fase++) {
    const nodi = algoritmo(fase + 1);
    const nodi2 = [];
    for (let i = 0; i < nodi.length; i += 2) {
      nodi2.push({
        id: FASI[fase].toLowerCase() + "-" + parseInt(i / 2),
        squadra1: nodi[i],
        squadra2: nodi[i + 1],
      });
    }
    data.push(nodi2);
  }
  return data;
}

export function calculateNodes(data) {
  const nodes = [];
  // Fase diretta
  const NUMERO_FASI = data.length;
  for (let fase = 0; fase < NUMERO_FASI; fase++) {
    // Header
    nodes.push({
      data: {
        type: "header",
        fase: FASI[fase],
      },
      style: {
        gridColumnStart: NUMERO_FASI - fase,
        gridRowStart: 1,
        gridRowEnd: 2,
      },
    });
    const nodi = data[fase];
    // Principal nodes
    nodes.push(
      ...nodi.map((v, i) => ({
        id: v.id,
        data: {
          type: "normal",
          squadra1: v.squadra1,
          squadra2: v.squadra2,
        },
        style: {
          gridColumnStart: NUMERO_FASI - fase,
          gridRowStart: i * 2 ** (NUMERO_FASI - fase - 1) + 2,
          gridRowEnd: "span " + 2 ** (NUMERO_FASI - fase - 1),
        },
        winner: 1,
      })),
    );
    if (fase == 0 && NUMERO_FASI > 1) {
      // Ripescaggio 2 (sotto la finale) ha un posizionamento non standard
      nodes.push({
        id: FASI[fase].toLowerCase() + "-rd-0",
        data: {
          type: "ripescaggio",
          number: 2,
        },
        style: {
          gridColumnStart: NUMERO_FASI,
          gridRowStart: 2 ** (NUMERO_FASI - 2) + 2,
          gridRowEnd: "span " + 2 ** (NUMERO_FASI - 2),
        },
      });
    }
    if (fase > 0 && NUMERO_FASI > 2 && fase < NUMERO_FASI - 1) {
      Array(2 ** (fase - 1))
        .fill(0)
        .forEach((_, i) => {
          nodes.push({
            id: FASI[fase].toLowerCase() + "-rd-" + i.toString(),
            data: {
              type: "ripescaggio",
              number: fase + 2,
            },
            style: {
              gridColumnStart: NUMERO_FASI - fase,
              gridRowStart: i * 2 ** (NUMERO_FASI - fase) + 2,
              gridRowEnd: "span " + 2 ** (NUMERO_FASI - fase),
            },
          });
        });
    }
  }

  const START = NUMERO_FASI + 1;

  // Fase ripescaggio
  for (let fase = 0; fase < NUMERO_FASI - 1; fase++) {
    for (let fase2 = 1; fase2 <= 2; fase2++) {
      nodes.push({
        data: {
          type: "header",
          fase: FASI[fase] + " R" + fase2,
        },
        style: {
          gridColumnStart: START + fase * 2 + fase2,
          gridRowStart: 1,
          gridRowEnd: 2,
        },
      });
      const nodi = data[fase];
      nodes.push(
        ...nodi.map((v, i) => ({
          id: FASI[fase].toLowerCase() + "-r" + fase2 + "-" + i.toString(),
          data: {
            type: "normal",
            squadra1: 1,
            squadra2: 2,
          },
          style: {
            gridColumnStart: START + fase * 2 + fase2,
            gridRowStart: i * 2 ** (NUMERO_FASI - fase - 1) + 2,
            gridRowEnd: "span " + 2 ** (NUMERO_FASI - fase - 1),
          },
          winner: 1,
        })),
      );
      if (fase == 0) continue;
      if (fase2 == 2) {
        Array(
          fase > 0 && fase < NUMERO_FASI - 1 ? Math.max(2 ** (fase - 2), 1) : 0,
        )
          .fill(0)
          .forEach((_, i) => {
            nodes.push({
              id: FASI[fase].toLowerCase() + "-rr2-" + i.toString(),
              data: {
                type: "ripescaggio",
                number: fase + 1,
              },
              style: {
                gridColumnStart: START + fase * 2 + fase2,
                gridRowStart: i * 2 ** (NUMERO_FASI - fase) + 2,
                gridRowEnd:
                  "span " + 2 ** (NUMERO_FASI - fase + (fase > 1 ? 1 : 0)),
              },
            });
          });
      }
    }
  }

  // Finalissima
  nodes.push({
    data: {
      type: "header",
      fase: "Finalissima",
    },
    style: {
      gridColumnStart: NUMERO_FASI + 1,
      gridRowStart: 1,
      gridRowEnd: 2,
    },
  });
  nodes.push({
    id: "finalissima",
    data: {
      type: "normal",
      squadra1: 1,
      squadra2: 2,
    },
    style: {
      gridColumnStart: NUMERO_FASI + 1,
      gridRowStart: 2,
      gridRowEnd: "span " + 2 ** (NUMERO_FASI - 1),
    },
    winner: 1,
  });
  nodes.push({
    id: "finali-r-0",
    data: {
      type: "ripescaggio",
      number: 1,
    },
    style: {
      gridColumnStart: NUMERO_FASI + 1,
      gridRowStart: 1 * 2 ** (NUMERO_FASI - 2) + 2,
      gridRowEnd: "span " + 2 ** (NUMERO_FASI - 2),
    },
  });
  nodes.push({
    id: "finali-rr2-0",
    data: {
      type: "ripescaggio",
      number: 1,
    },
    style: {
      gridColumnStart: NUMERO_FASI + 3,
      gridRowStart: 1 * 2 ** (NUMERO_FASI - 2) + 2,
      gridRowEnd: "span " + 2 ** (NUMERO_FASI - 2),
    },
  });

  // Ripescaggio ultima riga
  if (NUMERO_FASI > 1) {
    Array(Math.max(2 ** (NUMERO_FASI - 3), 1))
      .fill(0)
      .forEach((_, i) => {
        nodes.push({
          id: FASI[NUMERO_FASI - 1].toLowerCase() + "-rr2-" + i.toString(),
          data: {
            type: "ripescaggio",
            number: NUMERO_FASI,
          },
          style: {
            gridColumnStart: START + (NUMERO_FASI - 1) * 2 + 1,
            gridRowStart: i * 4 + 2,
            gridRowEnd: "span " + 4,
          },
        });
      });
  }
  return nodes;
}

export function calculateEdges(data) {
  const edges = [];
  const NUMERO_FASI = data.length;
  // Fase diretta
  for (let fase = 1; fase < NUMERO_FASI; fase++) {
    Array(2 ** fase)
      .fill(0)
      .forEach((_, i) => {
        const source = FASI[fase].toLowerCase() + "-" + i.toString();
        const target = FASI[fase - 1].toLowerCase() + "-" + parseInt(i / 2);
        edges.push({
          id: source + "__" + target,
          source,
          target,
          sourceHandle: i % 4 < 2 ? "top" : "bottom",
          targetHandle: i % 2 ? "bottom" : "top",
          sourcePosition: Position.Right,
          targetPosition: Position.Left,
          className: "green",
        });
      });
    Array(2 ** fase)
      .fill(0)
      .forEach((_, i) => {
        const source = FASI[fase].toLowerCase() + "-" + i.toString();
        const target = FASI[fase - 1].toLowerCase() + "-rd-" + parseInt(i / 4);
        edges.push({
          id: source + "__" + target,
          source,
          target,
          sourceHandle: i % 4 < 2 ? "bottom" : "top",
          targetHandle: (fase == 1 ? i % 2 < 1 : i % 4 < 2) ? "top" : "bottom",
          sourcePosition: Position.Right,
          targetPosition: Position.Left,
          className: "red",
        });
      });
  }

  // Fase ripescaggio
  for (let fase = 0; fase < NUMERO_FASI - 1; fase++) {
    for (let fase2 = 1; fase2 <= 2; fase2++) {
      if (fase == 0 && fase2 == 1) continue;
      Array(2 ** fase)
        .fill(0)
        .forEach((_, i) => {
          let source, target;
          if (fase2 == 1) {
            source = FASI[fase].toLowerCase() + "-r1-" + i.toString();
            target = FASI[fase - 1].toLowerCase() + "-r2-" + parseInt(i / 2);
          } else {
            source = FASI[fase].toLowerCase() + "-r2-" + i.toString();
            target = FASI[fase].toLowerCase() + "-r1-" + i;
          }
          edges.push({
            id: source + "__" + target,
            source,
            target,
            sourceHandle: "middle",
            targetHandle: (fase2 == 1 || fase == 1 ? i % 2 < 1 : i % 4 < 2)
              ? "top"
              : "bottom",
            sourcePosition: Position.Left,
            targetPosition: Position.Right,
            className: "green",
          });
        });
      if (fase2 == 2) {
        // Dai nodi ripescaggio
        Array(2 ** fase)
          .fill(0)
          .forEach((_, i) => {
            const source =
              FASI[fase].toLowerCase() + "-rr2" + "-" + parseInt(i / 4);
            const target =
              FASI[fase].toLowerCase() + "-r1" + "-" + i.toString();
            edges.push({
              id: source + "__" + target,
              source,
              target,
              sourceHandle:
                fase == 0
                  ? "middle"
                  : (fase == 1 ? i % 2 < 1 : i % 4 < 2)
                    ? "top"
                    : "bottom",
              targetHandle: (fase == 1 ? i % 2 < 1 : i % 4 < 2)
                ? "bottom"
                : "top",
              sourcePosition: Position.Left,
              targetPosition: Position.Right,
              className: "red",
            });
          });
      }
    }
  }

  // Finalissima
  edges.push({
    id: "finali-0" + "__" + "finalissima",
    source: "finali-0",
    target: "finalissima",
    sourceHandle: "top",
    targetHandle: "middle",
    sourcePosition: Position.Right,
    targetPosition: Position.Left,
    className: "green",
  });
  edges.push({
    id: "finali-r1-0" + "__" + "finalissima",
    source: "finali-r1-0",
    target: "finalissima",
    sourceHandle: "middle",
    targetHandle: "middle",
    sourcePosition: Position.Left,
    targetPosition: Position.Right,
    className: "green",
  });

  edges.push({
    id: "finali-0" + "__" + "finali-r-0",
    source: "finali-0",
    target: "finali-r-0",
    sourceHandle: "bottom",
    targetHandle: "middle",
    sourcePosition: Position.Right,
    targetPosition: Position.Left,
    className: "red",
  });

  // Ripescaggio ultima riga
  if (NUMERO_FASI > 1) {
    Array(2 ** (NUMERO_FASI - 1))
      .fill(0)
      .forEach((_, i) => {
        const source =
          FASI[NUMERO_FASI - 1].toLowerCase() + "-rr2" + "-" + parseInt(i / 4);
        const target =
          FASI[NUMERO_FASI - 2].toLowerCase() + "-r2" + "-" + parseInt(i / 2);
        edges.push({
          id: source + "__" + target,
          source,
          target,
          sourceHandle: i % 4 < 2 ? "top" : "bottom",
          targetHandle: i % 2 < 1 ? "top" : "bottom",
          sourcePosition: Position.Left,
          targetPosition: Position.Right,
          className: "red",
        });
      });
  }
  return edges;
}

function calculateCoords(viewportCoords, coords, position, handle) {
  let x = coords["x"] - viewportCoords["x"];
  if (position == Position.Right) x += coords["width"];
  let y = coords["y"] - viewportCoords["y"];
  switch (handle) {
    case "top":
      y += coords["height"] * 0.33;
      break;
    case "middle":
      y += coords["height"] / 2;
      break;
    case "bottom":
      y += coords["height"] * 0.66;
      break;
  }
  return [x, y];
}

export function calculateEdgeCoords(edges) {
  const paths = [];
  const viewportEl = document.getElementById("viewport");
  const viewportCoords = viewportEl.getBoundingClientRect();
  for (const edge of edges) {
    const source = viewportEl.querySelector("#" + edge.source);
    const target = viewportEl.querySelector("#" + edge.target);
    const sourceCoords = source.getBoundingClientRect();
    const targetCoords = target.getBoundingClientRect();
    const [sourceX, sourceY] = calculateCoords(
      viewportCoords,
      sourceCoords,
      edge.sourcePosition,
      edge.sourceHandle,
    );
    const [targetX, targetY] = calculateCoords(
      viewportCoords,
      targetCoords,
      edge.targetPosition,
      edge.targetHandle,
    );
    const [path] = getBezierPath({
      sourceX,
      sourceY,
      targetX,
      targetY,
      sourcePosition: edge.sourcePosition,
      targetPosition: edge.targetPosition,
    });
    paths.push({
      path,
      style: edge.style,
      id: edge.id,
      className: edge.className,
    });
  }
  return paths;
}
