...
export function calcClassificaAvulsa(records) {
  ...
  Object.keys(squadre).forEach((v) => {
    ...
    if (partiteGiocate > 0) {
      squadre[v][EnumClassificaAvulsa.QuozientePunteggio] =
        squadre[v][EnumClassificaAvulsa.Punti] / partiteGiocate;
    }
    squadre[v][EnumClassificaAvulsa.QuozientePunti] =
      squadre[v][EnumClassificaAvulsa.PuntiFatti] /
        squadre[v][EnumClassificaAvulsa.PuntiSubiti] || null;
  });

  return Object.values(squadre).sort((a, b) => {
    if (
      a[EnumClassificaAvulsa.QuozientePunteggio] !=
      b[EnumClassificaAvulsa.QuozientePunteggio]
    ) {
      return (
        b[EnumClassificaAvulsa.QuozientePunteggio] -
        a[EnumClassificaAvulsa.QuozientePunteggio]
    ...
    if (
      a[EnumClassificaAvulsa.QuozientePunti] !=
      b[EnumClassificaAvulsa.QuozientePunti]
    ) {
      return (
        b[EnumClassificaAvulsa.QuozientePunti] -
        a[EnumClassificaAvulsa.QuozientePunti]
