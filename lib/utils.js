import { DOPPIA_ELIMINAZIONE } from "./const.js";
...
export function whoIsWinner(row) {
  const punti1 = parseInt(row["Punti 1"]);
  const punti2 = parseInt(row["Punti 2"]);
...
export function calculateMaxFase(data) {
  return Object.keys(Object.groupBy(data, (v) => [v.Fase])).length - (DOPPIA_ELIMINAZIONE ? 1 : 0);
}
